...
- ## 错误传递
	- 使用`errors.Wrap/Wrapf` 传递必要的上下文,尽量使上下文简洁,否则当调用链过长时,整个报错信息会非常冗余繁琐
		  ```go
		  ...
		  func getUserInfo(name string, filePath string) ([]string, error) {
		  	...
		  	if err != nil {
		  		return nil, errors.Wrapf(err, "readFile,name:%s", name)
		  	}
		...
		- 使用`fmt.Printf("get user info error: %v\n", err)` 输出如下,错误由外层到内层
			- `get user info error: readFile,name:张三: open file: open ./user.txt: no such file or directory`
		- 使用`fmt.Printf("get user info error: %+v\n", err)` 输出如下,错误由内层到外层
			  ```
			  get user info error: open ./user.txt: no such file or directory
			  open file
			  github.com/mtgnorton/k/kerror.readFile
			          /Users/mtgnorton/Coding/go/src/github.com/mtgnorton/k/kerror/kerror_test.go:46
			  github.com/mtgnorton/k/kerror.getUserInfo
			          /Users/mtgnorton/Coding/go/src/github.com/mtgnorton/k/kerror/kerror_test.go:27
			  github.com/mtgnorton/k/kerror.TestGetUserInfo
			          /Users/mtgnorton/Coding/go/src/github.com/mtgnorton/k/kerror/kerror_test.go:16
			  testing.tRunner
			  ...
			  readFile,name:张三
			  github.com/mtgnorton/k/kerror.getUserInfo
			          /Users/mtgnorton/Coding/go/src/github.com/mtgnorton/k/kerror/kerror_test.go:29
			  github.com/mtgnorton/k/kerror.TestGetUserInfo
			          /Users/mtgnorton/Coding/go/src/github.com/mtgnorton/k/kerror/kerror_test.go:16
			  testing.tRunner
			          /Users/mtgnorton/.g/go/src/testing/testing.go:1690
			  ...
			  ```
		- #### 最佳实践
			- 每层调用`errrors.Wrap/Wrapf`时,关键点如下
			- 消息为正在执行的操作,使用自然语言描述
			- 只包含该层的相关参数
			- 保持简洁,不需要在消息里面添加error关键词,否则error会出现很多次,造成冗余
			- 举例
				- 如在上例中` errors.Wrapf(err, "readFile,name:%s", name)`
				- 反例1:
					- `errors.Wrapf(err, "getUserInfo readFile error ,name:%s", name)`, getUserInfo应该在上一层调用中体现,且error冗余
				- 反例2:
					- ` errors.Wrapf(err, "readFile ,name:%s,path:%s", name,path)`, path 应该由下层包含,不应该在本层包含
- ## 判断错误
	- 判断错误有以下几种方式
	- ### 等号判断
		- 适用于错误未被包装且是预定义错误的方式
